...
/*
func TestCombineCIDRs(t *testing.T) {
	testCases := []struct {
		name     string
		cidrs    []string
		expected []string
	...
	}{
		{
			...
			cidrs:    []string{"192.168.16.0/23", "192.168.18.0/24", "192.168.19.0/24", "192.168.20.0/22"},
			expected: []string{"192.168.16.0/21"},
			wantErr:  false,
		},
		{
			name:     "Non-Adjacent Blocks",
			cidrs:    []string{"192.168.16.0/24", "192.168.18.0/24"},
			expected: []string{"192.168.16.0/24", "192.168.18.0/24"},
			wantErr:  false,
		},
		{
			name:     "Single Block",
			cidrs:    []string{"192.168.16.0/24"},
			expected: []string{"192.168.16.0/24"},
			wantErr:  false,
		},
		{
			name:     "Empty Input",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := combineCIDRs(tc.cidrs)
			if (err != nil) != tc.wantErr {
				t.Errorf("combineCIDRs() error = %v, wantErr %v", err, tc.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tc.expected) {
				t.Errorf("%s combineCIDRs() = %v, want %v", tc.name, got, tc.expected)
			}
		})
	}
}
